...
const skillsAnalysisCache = {
    getKey: (resumeId) => `skills_analysis_${resumeId}`,

    get: (resumeId) => {
        try {
            const cached = sessionStorage.getItem(skillsAnalysisCache.getKey(resumeId));
    ...
    set: (resumeId, data) => {
        try {
            sessionStorage.setItem(skillsAnalysisCache.getKey(resumeId), JSON.stringify(data));
        } catch (error) {
    ...
    clear: (resumeId) => {
        sessionStorage.removeItem(skillsAnalysisCache.getKey(resumeId));
    }
};
...
function AIAnalysisModal({ 
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true);
                console.log('Processing resumeData:', resumeData);
                ...
                let analysisData = skillsAnalysisCache.get(resumeId);
                ...
                if (!analysisData) {
                    ...
                    if (analysisData) {
                        skillsAnalysisCache.set(resumeId, analysisData);
                    }
                } else {
                    console.log('Using cached skills analysis data');
                }

                if (analysisData) {
                    setVersions(analysisData);
    ...
    const handleImprove = async () => {
        try {
            ...
            setModalState(MODAL_STATES.ANALYSIS);

            // Update cache with new analysis
            if (response.analysis) {
                skillsAnalysisCache.set(resumeId, response.analysis);
            }
    ...
    const handleGenerateNew = () => {
        // Clear cache when generating new
        skillsAnalysisCache.clear(resumeId);
        setVersions(null);
    ...
    switch (modalState) {
        ...
        case MODAL_STATES.ANALYSIS:
            console.log('Rendering SkillsAnalysisResultModal');
            return (
        ...
        default:
            console.log('Rendering SkillsAnalysisModal');
            return (
                <SkillsAnalysisModal 
                    isOpen={isOpen}
                    onClose={onClose}
...
export function skillsAnalysisModal({ currentContent, resumeId, onVersionSelect, resumeData }) {
    ...
    console.log('skillsAnalysisModal called with:', {
        ...
        resumeDataContent: {
            ...
            hasAnalysis: !!resumeData?.skills_analysis,
            improvedSections: resumeData?.ai_improved_sections
        }
    });
